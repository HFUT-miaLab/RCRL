...
# local library
from RCRL.Dataset_TCGA_EGFR import TCGA_EGFR_feat_label_Dataset, get_train_valid_names, get_test_names
import metric
from lookahead import Lookahead
from RCRL.models.model import RCRL
...
def eval(args, test_loader, model, eval_metric='ACC'):
    ...
    with torch.no_grad():
        for step, (x,text_feature,text_tokens_feature,sequence_feature, target) in enumerate(test_loader):
            if torch.cuda.is_available():   #若batch_size不为1则需自行调整
                x = x.cuda()
                text_feature = text_feature.squeeze().unsqueeze(0).cuda()
                text_tokens_feature = text_tokens_feature.squeeze().unsqueeze(0).cuda()
                sequence_feature = sequence_feature.squeeze().unsqueeze(0).cuda()
                target = target.cuda()
...
def train(args, model, train_loader, valid_loader):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for step, (x, text_feature,text_tokens_feature,sequence_feature, target) in enumerate(train_loader):
            ...
            image_loss = compute_contrastive_loss(results_dict['fusion_probs'], results_dict['length_sequence'])
            sequence_loss = compute_contrastive_loss(results_dict['sequence_probs'], results_dict['length_sequence'])
            L_mol = (image_loss + sequence_loss) / 2.0

            original_loss = compute_contrastive_loss(results_dict['original_probs'], results_dict['length_reconstruct'])
            reconstruct_loss = compute_contrastive_loss(results_dict['reconstruct_probs'], results_dict['length_reconstruct'])
