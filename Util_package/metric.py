...
def compute_specificity(y, pred_labels):
    assert len(y) == len(pred_labels)
...
def compute_sensitivity(y, pred_labels):
    assert len(y) == len(pred_labels)
...
def draw_confusion_matrix(y, pred_labels, class_names, save_path=None):
    """Draw binary-class confusion matrix.

    Parameters
    ----------
    y : ground truth labels list'

    pred_labels : pred labels list.
    ...
    --------
        ...
        draw_confusion_matrix(result['true_labels'], result['pred_labels'])
